...
cursor.execute('''INSERT INTO "Identity"("Name", "ARN", "Type") VALUES('{0}', '{1}', '{2}')'''.format(IAM_user, user_arn, "User"))
cursor.execute('''INSERT INTO "Assume"("IAM_UserARN", "IdentityARN") VALUES('{0}', '{1}')'''.format(user_arn, user_arn))
conn.commit()
...
for g in groups['Groups']:
    cursor.execute('''INSERT INTO "Identity"("Name", "ARN", "Type") VALUES('{0}', '{1}', '{2}')'''.format(g['GroupName'], g['Arn'], "Group"))
    cursor.execute('''INSERT INTO "Assume"("IAM_UserARN", "IdentityARN") VALUES('{0}', '{1}')'''.format(user_arn, g['Arn']))
    conn.commit()
...
def get_role_policies(policy):
    ...
    if 'PolicyDocument' in policy:
        ...
        role_name.strip()
        if role_name != "":
            cursor.execute('''INSERT INTO "Identity"("Name", "ARN", "Type") VALUES('{0}', '{1}', '{2}')'''.format(role_name, role_arn, "Role"))
            cursor.execute('''INSERT INTO "Assume"("IAM_UserARN", "IdentityARN") VALUES('{0}', '{1}')'''.format(user_arn, role_arn))
            conn.commit()
            ...
            for rpolicy in role_policies['AttachedPolicies']:
                ...
                cursor.execute('''INSERT INTO "Assign"("IdentityARN", "Policyid") VALUES('{0}', {1})'''.format(role_arn, id))
                conn.commit()
...
for ipolicy in inline_policies['PolicyNames']:
    ...
    id = cursor.fetchone()[0]
    cursor.execute('''INSERT INTO "Assign"("IdentityARN", "Policyid") VALUES('{0}', {1})'''.format(user_arn, id))
    conn.commit()
...
if groups != []:  
    for group in groups['Groups']: 
        for gpolicy in group_policies['AttachedPolicies']:
            ...
            id = cursor.fetchone()[0]
            cursor.execute('''INSERT INTO "Assign"("IdentityARN", "Policyid") VALUES('{0}', {1})'''.format(group['Arn'], id))
            conn.commit()
            cursor.execute('''INSERT INTO "Statement"("WholeStatement", "Statementid") VALUES('{0}', {1})'''.format(json.dumps(policy_version['Document']['Statement']), id))
            conn.commit()
